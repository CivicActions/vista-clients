...
class TestTransportConnect:
    def test_connect_success(self):
        with patch("vista_clients.rpc.transport.socket.socket") as mock_cls:
            mock_sock = MagicMock()
    ...
    def test_connect_refused_raises_connection_error(self):
        with patch("vista_clients.rpc.transport.socket.socket") as mock_cls:
            ...
            with pytest.raises(BrokerConnectionError, match="refused"):
                t.connect()
            assert not t.is_connected
...
class TestTransportSend:
    ...
    def test_send_broken_connection_raises(self):
        ...
        with pytest.raises(BrokerConnectionError, match="Send failed"):
            t.send(b"data")
        assert not t.is_connected
...
class TestTransportReceive:
    ...
    def test_receive_timeout_raises(self):
        ...
        with pytest.raises(BrokerConnectionError, match="Receive timed out"):
            t.receive()
    ...
    def test_receive_connection_closed_by_server(self):
        ...
        with pytest.raises(BrokerConnectionError, match="closed by server"):
            t.receive()
    ...
    def test_receive_mid_receive_drop(self):
        ...
        with pytest.raises(BrokerConnectionError, match="Receive failed"):
            t.receive()
        assert not t.is_connected

...
**Tests**: Included — smoke tests against a reference VistA environment (VEHU) are required by SC-006.
...
- **Single project**: `src/vista_clients/rpc/`, `tests/` at repository root
...
- [X] T001 Create project directory structure: `src/vista_clients/rpc/`, `tests/unit/`, `tests/contract/`, `tests/smoke/` with `__init__.py` files
...
- [X] T004 [P] Implement exception hierarchy (`VistAError`, `ConnectionError`, `HandshakeError`, `AuthenticationError`, `ContextError`, `RPCError`, `StateError`) in `src/vista_clients/rpc/errors.py`
- [X] T005 [P] Implement enumerations (`ParamType`, `SessionState`, `CredentialSource` [internal]) and data classes (`RPCParameter`, `RPCResponse`) with factory functions (`literal()`, `list_param()`) in `src/vista_clients/rpc/protocol.py` (types-only scaffold; message building comes in US2)
- [X] T006 Implement protocol encoding primitives (`spack()`, `lpack()`) in `src/vista_clients/rpc/protocol.py`. L-PACK must use 3-digit format for values ≤999 chars and 5-digit format for values >999 chars (per XWB*1.1*65)
...
- [X] T008 Configure logging with credential redaction helper in `src/vista_clients/rpc/broker.py` (module-level logger setup only; class comes in US1)
...
- [X] T009 [US1] Implement `Transport` class (connect, send, receive with `chr(4)` framing, close, `is_connected` property) in `src/vista_clients/rpc/transport.py`
- [X] T010 [US1] Add broken-connection detection (socket errors during send/receive raise `ConnectionError`) in `src/vista_clients/rpc/transport.py`
- [X] T011 [US1] Implement `VistABroker` class scaffold with constructor, `connect()` (TCP-only, no handshake yet), `disconnect()`, `is_connected`, `state` property, and context manager (`__enter__`/`__exit__`) in `src/vista_clients/rpc/broker.py`
...
- [X] T014 [P] [US2] Implement OSEHRA cipher table, `encrypt()` and `decrypt()` functions in `src/vista_clients/rpc/protocol.py`
- [X] T015 [P] [US2] Add unit tests for cipher `encrypt()`/`decrypt()` round-trip and known test vectors in `tests/unit/test_protocol.py`
- [X] T016 [US2] Implement `build_connect_message()` (TCPConnect command with hostname and app_name) in `src/vista_clients/rpc/protocol.py`
- [X] T017 [US2] Implement `build_rpc_message()` for RPC invocation (protocol prefix, S-PACK name, parameter encoding for literal and list types) in `src/vista_clients/rpc/protocol.py`
- [X] T018 [US2] Implement `build_disconnect_message()` (`#BYE#` RPC) in `src/vista_clients/rpc/protocol.py`
...
- [X] T021 [US2] Update `VistABroker.connect()` to include XWB handshake: send TCPConnect, validate "accept" response, transition to HANDSHAKED state in `src/vista_clients/rpc/broker.py`
- [X] T022 [US2] Implement `VistABroker.create_context()` with option name encryption and `XWB CREATE CONTEXT` RPC in `src/vista_clients/rpc/broker.py`
- [X] T023 [US2] Add session state enforcement: reject operations in invalid states, raise `StateError` in `src/vista_clients/rpc/broker.py`
- [X] T024 [US2] Update `VistABroker.disconnect()` to send `#BYE#` before closing socket in `src/vista_clients/rpc/broker.py`
...
**Independent Test**: Connect to VEHU, handshake, authenticate with built-in demonstration defaults, verify DUZ is returned.

**Covers**: FR-005, FR-006, FR-009 (auth errors), FR-015 (credential redaction in logs)

### Implementation for User Story 3

- [X] T026 [US3] Implement credential resolution logic (explicit → env vars → built-in demonstration defaults) in `src/vista_clients/rpc/broker.py`
- [X] T027 [US3] Implement `VistABroker.authenticate()`: send `XUS SIGNON SETUP`, encrypt credentials, send `XUS AV CODE`, parse DUZ from response, transition to AUTHENTICATED state in `src/vista_clients/rpc/broker.py`
- [X] T028 [US3] Add credential redaction to all logging statements (replace access/verify values with `***REDACTED***`) in `src/vista_clients/rpc/broker.py`
...
- [X] T030 [US3] Add smoke test: authenticate with built-in demonstration defaults, verify DUZ > 0 returned in `tests/smoke/test_lifecycle.py`
...
- [X] T032 [US4] Implement `VistABroker.call_rpc()`: build RPC message, send via transport, receive raw response, return `RPCResponse` in `src/vista_clients/rpc/broker.py`
- [X] T033 [US4] Implement `VistABroker.ping()`: send `XWB IM HERE` RPC for keepalive in `src/vista_clients/rpc/broker.py`
- [X] T034 [US4] Add wire-level DEBUG logging to `call_rpc()` (bytes sent/received, with credential redaction) in `src/vista_clients/rpc/broker.py`
...
- [X] T039 [US5] Implement `parse_response()` function: extract security/error prefix packets (per XWBRW.m `SNDERR`), raise `RPCError` if non-empty, then detect single vs array response, return `RPCResponse` with `value` or `lines` populated in `src/vista_clients/rpc/protocol.py`
- [X] T040 [US5] Wire `parse_response()` into `VistABroker.call_rpc()` return path in `src/vista_clients/rpc/broker.py`
...
- [X] T044 [P] Create public re-exports in `src/vista_clients/rpc/__init__.py` per contracts/api.md
- [X] T045 [P] Add Google-style docstrings to all public classes, methods, and functions in `src/vista_clients/rpc/broker.py`, `src/vista_clients/rpc/protocol.py`, `src/vista_clients/rpc/transport.py`, `src/vista_clients/rpc/errors.py`
...
- [X] T047 Validate quickstart.md code examples execute against a reference VistA environment (VEHU) (SC-001: 10-line usage) in `tests/smoke/test_quickstart.py`
...
- [X] T051 Run full `tests/smoke/` suite against a reference VistA environment (VEHU) and confirm 100% pass rate (SC-006). Verify FR-016 by grepping `src/vista_clients/rpc/` for `retry|retries|attempt` and asserting zero matches
...
Task T004: "Implement exception hierarchy in src/vista_clients/rpc/errors.py"
Task T005: "Implement enumerations and data classes in src/vista_clients/rpc/protocol.py"
...
Task T014: "Implement cipher encrypt/decrypt in src/vista_clients/rpc/protocol.py"
...
4. **STOP and VALIDATE**: Test TCP connect/disconnect against a reference VistA environment (VEHU)
